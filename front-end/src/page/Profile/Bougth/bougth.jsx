...
function Bougth() {
  ...
  const img =
    useSelector((state) => state.billAccount.data?.products?.images) || item_cf;
  ...
  const fetchNotification = async (sortOption) => {
    ...
    const result = await GetBill_AccountAPI(queryParams);
    dispatch(initDataBillAccount(result.data));
  };
